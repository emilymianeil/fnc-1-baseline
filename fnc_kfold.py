...
from keras.layers import Input, LSTM, Embedding, Dense, Flatten, Dropout, Activation, Lambda, Bidirectional, Conv1D, MaxPooling1D, GlobalMaxPool1D, SpatialDropout1D
from tensorflow.keras.preprocessing.text import text_to_word_sequence
...
if __name__ == "__main__":
    ...
    clfType = GradientBoostingClassifier(n_estimators=200, learning_rate=0.2, random_state=14128,
    ...
    for fold in fold_stances:
        ...
        # train model
        print("START")
        y_train_rel = np.asarray(y_train_rel).astype('float32').reshape((-1, 1))
        # X_shortened_train = np.asarray(X_shortened_train).astype('float32').reshape((-1, 1))
        ...
        # predsRel = modelRel.predict(X_test)
        type = []

        # predsType = modelType.predict(np.asarray(X_test_type))
        predsType = clfType.predict(X_test_type)
    ...
    predsType = clfType.predict(X_holdout_rel)
    #  = modelType.predict(np.asarray(X_holdout_type))
    ind = 0
    predicted_holdout = []
    for i, p in enumerate(predsRel):
        prediction = 0
        if p < 0.5:
            ...
            prediction = 3
        predicted_holdout.append(LABELS[int(prediction)])
    ...
    predsType = clfType.predict(X_competition_type)
    ind = 0
    ...
    for i, p in enumerate(predsRel):
        ...
        if p < 0.5:
            ...
            prediction = 3
        predicted_competition.append(LABELS[int(prediction)])
